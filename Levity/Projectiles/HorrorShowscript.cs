...
namespace Levity.Projectiles //We need this to basically indicate the folder where it is to be read from, so you the texture will load correctly 

{
	public class HorrorShowProjectile : ModProjectile
	{ 
		...
		{ 
			...
			projectile.light = 0.9f; //Lights up the whole room projectile.alpha = 128; //Semi Transparent
			projectile.rotation += (float)projectile.direction * 0.8f; //Spins in a good speed 
			int DustID = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y + 2f), projectile.width + 20, projectile.height + 20, 36, projectile.velocity.X * 0.2f, projectile.velocity.Y * 0.2f, 120, default(Color), 0.75f); //Spawns dust 
			Main.dust[DustID].noGravity = true; //Makes dust not fall 
			if(projectile.timeLeft % 100 == 0) //If the remainder of the timeLeft divided 15 is 0, then make the projectile; Every 15 seconds, projectile spawns another projectile 
			{
		...
		{ 
			...
			if(rand == 0) { n.AddBuff(24, 180); //On Fire! debuff for 3 seconds 
			} else if (rand == 1) { owner.statLife += 5; //Gives 5 Health 
		...
		{ 
			...
			if(rand == 0) 
			{
				Item.NewItem((int)projectile.position.X, (int)projectile.position.Y, projectile.width, projectile.height, mod.ItemType("HorrorShowProjectile"), 1, false, 0, false); //Spawns a bullet 
			} 
		}
	} 
}
